...
const QueryFilter = ({ title, typeInput, placeholder, handleData }: QueryFiltroProps) => {
  ...
  return (
    <div className={styles.query_filtro}>
      ...
      {typeInput === "search" && (
        <InputSearch id={title} placeholder={placeholder} onChange={(data: any) => setSearchInputValue(data)} />
      )}
      ...
      {typeInput === "date" && (
        <InputDate name={title} placeholder="Ano" onChange={(year: any) => setYearInputValue(year)} />
      )}
    </div>
  );
};
